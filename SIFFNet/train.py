...
train_path_x = "../SIFFNet/data/feature_VSP_0621/"  # Feature data path
train_path_y = "../SIFFNet/data/label_VSP_0621/"  # Label data path
train_path_z = "../SIFFNet/data/mask_VSP_0621/"  # Mask data path
...
# Learning rate scheduler (reduce LR after 20 epochs)
def lr_lambda(epoch):
    if epoch < 20:
        return 1.0
    else:
        return 0.5 ** ((epoch - 20) // 20 + 1)
...
for epoch in range(epochs):
    ...
    for batch_idx, (batch_x, batch_y, batch_z) in enumerate(train_loader):
        ...
        # Loss calculation (combination of MSE , BCE losses and SSIM loss)
        loss22 = criterion3(err_out, batch_z)
        loss11 = criterion1(err_out1, (batch_x - batch_y))+0.001*criterion2(err_out1,(batch_x-batch_y))
    ...
    with torch.no_grad():
        for val_x, val_y, val_z in valida_loader:
            ...
            loss111 = criterion1(err_out2, (val_x - val_y))+ 0.001 * criterion2(err_out2, (val_x - val_y))
    ...
    torch.save(my_net, os.path.join('E:/jiaojie/SIFFNet/model', model_name + '.pth'))
